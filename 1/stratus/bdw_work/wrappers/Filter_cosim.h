...
#ifndef _FILTER_COSIM_INCLUDED_
#define _FILTER_COSIM_INCLUDED_
...
#if defined(XM_SYSTEMC) || defined(NC_SYSTEMC)
struct Filter_cosim : public xmsc_foreign_module
#else
SC_MODULE(Filter_cosim)
#endif
{
	struct StringPair {
    ...
    void end_of_elaboration()
    {
		...
		if ( nin == 0 )
		{
			if ( numLinked > 0 ) 
			{
				esc_report_error( esc_fatal, 
					"More than one instance of module Filter is being simulated in Verilog,\n"
					...
					"where <inst1> and <inst2> are the leaf names of instances of Filter in SystemC\n", 
					scName );
				return;
			} else {
				s << "Filter0" << std::ends;
		...
		else
		{
			...
			{
				esc_report_error( esc_fatal, 
								  "More than %d instance(s) of module Filter are being simulated in Verilog,\n"
    ...
    void link_signals(const char *module_path, const char *sim_domain)
    {
		...
		if ( i_clk_is_clock )
		{
			...
			++registeredClocks;
		} else if ( qbhGetPortUsedAsClock( qbhEmptyHandle, "Filter", "i_clk", scName ) ) {
			esc_report_error( esc_error, "The port 'i_clk' on module 'Filter'\n\tis used as a clock in the design,\n\tbut is not connected to an sc_clock");
		}
		if ( i_rst_is_clock )
		{
			...
			++registeredClocks;
		} else if ( qbhGetPortUsedAsClock( qbhEmptyHandle, "Filter", "i_rst", scName ) ) {
			esc_report_error( esc_error, "The port 'i_rst' on module 'Filter'\n\tis used as a clock in the design,\n\tbut is not connected to an sc_clock");
		}
		if ( i_rgb_vld_is_clock )
		{
			...
			++registeredClocks;
		} else if ( qbhGetPortUsedAsClock( qbhEmptyHandle, "Filter", "vld", scName ) ) {
			esc_report_error( esc_error, "The port 'vld' on module 'Filter'\n\tis used as a clock in the design,\n\tbut is not connected to an sc_clock");
		}
		if ( o_result_busy_is_clock )
		{
			...
			++registeredClocks;
		} else if ( qbhGetPortUsedAsClock( qbhEmptyHandle, "Filter", "busy", scName ) ) {
			esc_report_error( esc_error, "The port 'busy' on module 'Filter'\n\tis used as a clock in the design,\n\tbut is not connected to an sc_clock");
		}
		...
		{
			esc_report_error( esc_fatal, 
				"The clock port in module Filter must have\n"
    ...
    const char* hdl_name() const 
    {
        ...
        {
            return "Filter_nc_cosim"; 
        }
    } 
    Filter_cosim( sc_module_name name )
        : xmsc_foreign_module(name)
#else
	Filter_cosim( sc_module_name in_name=sc_module_name(sc_gen_unique_name(" Filter") ) )
		: sc_module(in_name)
#endif
		  ,i_clk("i_clk")
		  	,i_rst("i_rst")
...
int Filter_cosim::numLinked = 0;
Filter_cosim::StringPair Filter_cosim::instanceNames[] = {
	{ NULL, NULL, false } };
